...
async function getPortfolios(authToken) {
  ...
  const response = await fetch(`${BASE_URL}/api/portfolios/me`, {
    method: 'GET',
    headers: {
  ...
  if (!data.success) {
    throw new Error(`Failed to fetch portfolios: ${data.message}`);
  }
...
async function publishPortfolio(authToken, portfolioId, subdomain) {
  ...
  const response = await fetch(`${BASE_URL}/api/site/publish`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${authToken}`,
      'Content-Type': 'application/json'
