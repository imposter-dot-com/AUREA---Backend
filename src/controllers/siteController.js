...
// Import service modules - USING NEW HTML GENERATOR
import { generateExactHTML } from '../services/generateExactHTML.js';
// NOTE: CSS is now included in the generateExactHTML function
...
// Function to process portfolio data (copied from generateExactHTML.js service)
...
export const debugGenerate = async (req, res) => {
  try {
    ...
    // Generate HTML content - USING NEW HTML GENERATOR (includes CSS inline)
    // Use the main generateExactHTML function with user's portfolio data
    const htmlContent = generateExactHTML(portfolio);
    const cssContent = '/* CSS is included inline in HTML */'; // Placeholder for compatibility

    // Create deployment files structure
    const deploymentFiles = generateDeploymentFiles(
      { 
...
export const publishSite = async (req, res) => {
  try {
    ...
    // Generate subdomain from user's name in hero section
    let subdomain = generateSubdomainFromHero(portfolio);
    ...
    const deploymentFiles = generateDeploymentFiles(siteConfig, htmlContent, cssContent);
